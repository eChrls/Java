...
// 1- Dada una lista de números enteros, obtener una nueva lista que contenga solo los números pares.
...
// 2- Dada una lista de números enteros, calcular la suma de todos los elementos.  
 ...
 // 3-  Realiza el ejercicio anterior usando la función reduce.  
    //reduce(T, BinaryOperator<T>) realiza la reducción usando un valor inicial y una función asociativa. Se devuelve un valor como resultado.
    //streamOperaciones esta consumido, produce excepcion. 
    IntStream streamSumaReduce = IntStream.of(array);
        int suma = streamSumaReduce.reduce(0, (acumulador, numero)-> acumulador + numero); // acumulador += numero
        System.out.println("Suma con reduce: " + suma);
        System.out.println("\n");
        
 
// 4- Dada una lista de números enteros, obtener el numero mínimo o cero si la lista está vacía.
            System.out.println("Maximo : " + IntStream.of(array).max().orElse(0)); // orElse devuelve 0 si esta vacio
...
// 5- Dada una lista de números enteros, obtener una nueva lista que contenga solo los números impares multiplicados por dos.
...
// 7- Dada una lista de cadenas, obtener una nueva lista que contenga las mismas cadenas, pero en mayúsculas. 
        List<String>listaNombres = listaEmpleados.stream().map(Empleado::nombre)
                                                                                        .collect(Collectors.toList()); //Conversión a lista
       
        Stream<String> enMayus = listaNombres.stream().map(cadena -> cadena.toUpperCase());
...
// 9- Imprimir por pantalla la longitud de cada string de la lista de cadenas original.
        ...
        listaNombres.stream()
                                        .map(nombre -> nombre + " : " + nombre.replaceAll(" ", "").length()) //reemplaza un espacio por nada
                                        .forEach(System.out::println);
                                        System.out.println("\n");
                                       
// 10 - Sobre la lista de cadenas original, obtener una nueva lista que contenga solo las cadenas que comienzan con la letra "A", 
...
// 12 - Ordenar la lista por salario de forma descendente. Usa reversed en el objeto Comparator. 
...
// 13- Ordenar la lista por nombre y obtener una nueva lista con los tres primeros elementos.
...
// 14 - Comprobar si algún empleado tiene salario superior a 100.  *Nota : Lo cambio a 3000 para filtrar. 
...
// 15 - Obtener un set con los nombres de los empleados mayores de 18 años. *Nota: Lo cambio a 40 años para filtrar.
