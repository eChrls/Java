...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        // Leer CSV y almacenar cursos
        List<Curso> cursos = LecturaCsv.leerCsv2();

        // Ordenar cursos por fecha de inicio y luego por título
        cursos.sort(ordenarFechaYtitulo());

        // Filtrar cursos cuya modalidad sea "Jornadas" (ignorando mayúsculas/minúsculas)
        // y cuya fecha de fin sea anterior al 01/01/2020. Luego imprimir con Iterator.
        Set<String> cursoSet = cursos.stream()
                .filter(c -> c.getModalidad().equalsIgnoreCase("Jornadas"))
                .filter(c -> c.getFechaFin().isBefore(LocalDate.of(2020, 1, 1)))
                .map(Curso::getTitulo)
        ...
        // Map con número de cursos TERMINADOS por cada centro
        Map<String, Long> cursosTerminadosPorCentro = cursos.stream()
                .filter(c -> c.getEstado().equalsIgnoreCase("Terminada"))
                .collect(Collectors.groupingBy(Curso::getCentro, Collectors.counting()));

        // Imprimir el map
        cursosTerminadosPorCentro.forEach((centro, count)
                -> System.out.println("Centro: " + centro + " - Cursos terminados: " + count));

        // Contar modalidades distintas
        long totalModalidades = cursos.stream()
                .map(Curso::getModalidad)
                .distinct()
                .count();

        System.out.println("Modalidades distintas: " + totalModalidades);
    }

// Ordenar por fecha de inicio, luego por título
